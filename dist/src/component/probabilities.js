...
exports.calculateConditionalProbabilities = exports.calculateClassPriors = void 0;
...
exports.calculateClassPriors = calculateClassPriors;
// Function to calculate conditional probabilities
function calculateConditionalProbabilities(data, feature, value, className) {
    const instancesWithClass = data.filter((instance) => instance.stroke === className);
    const totalInstancesWithClass = instancesWithClass.length;
    const instancesWithClassAndValue = instancesWithClass.filter((instance) => instance[feature] === value).length;
    const uniqueFeatureValues = new Set(data.map((instance) => instance[feature]));
    return ((instancesWithClassAndValue + 1) /
        (totalInstancesWithClass + uniqueFeatureValues.size));
}
exports.calculateConditionalProbabilities = calculateConditionalProbabilities;
